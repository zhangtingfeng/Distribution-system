<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="tab_Order_Board_NeedMoney.aspx.cs" Inherits="_05Client.eggsoft.cn.ClientAdmin._19tab_Order.tab_Order_Board_NeedMoney" %>
...
<body id="mybody">
    <form id="Form1" method="post" runat="server">
                ...
                <table id="DataList2" width="100%" border="0" cellpadding="0" cellspacing="0" class="border mtab">
                    <tr bgcolor="#0000CC" class="title" style="text-align: center;">
                        ...
                        <th width="8%" height="25" align="center">
                            <strong>用户信息</strong></th>
                        <th width="8%" height="25" align="center">
                            <strong>用户地址</strong></th>
                        <th width="15%" height="25" align="center">
                            <strong>订单时间</strong></th>
                        <th width="15%" height="25" align="center">
                            <strong>订单金额</strong></th>
                        <th width="15%" height="25" align="center">
                            <strong>删除无效订单</strong></th>
                    </tr>
            </HeaderTemplate>
            <ItemTemplate>
                <tr bgcolor="#FFFFFF" class="tdbg">
                    ...
                    <td height="40" width="15%" class="styleOrder">
                        <%#getUserName(Eval("UserID").ToString(),Eval("ShopClient_ID").ToString())%> 编号ID:<%#Eval("ShopUserID").ToString()%></td>
                    <td height="40" width="15%" class="styleOrder">
                        <%#Eggsoft_Public_CL.GoodP.getAddress(Eval("User_Address").ToString(),Eval("O2OTakedID").ToString())%></td>
                    <td height="40" width="15%" class="styleOrder">
                    ...
                    <td height="40" width="15%" class="styleOrder"><a href="tab_Order_Manage.aspx?Type=delete&ID=<%#Eval("ID")%>&CallBackUrl=tab_Order_Board_NeedMoney.aspx*pageIndex=<%=Request.QueryString["pageIndex"]%>" onclick="return confirm('确定删除吗?')">删除</a>
                    </td>
                </tr>
                <tr bgcolor="#eeeeee" style="border-bottom: 1px dashed #000;" width="100%">
                    <td colspan="6" width="100%">
                        <!--<%#getColor(Eval("ID").ToString())%>-->
                        <asp:DataList ID='OrderDatail' EnableViewState='false' runat='server' CssClass="style100Percent" OnItemDataBound="DataList2_ItemDataBound" Width="100%">
                            <HeaderTemplate>
                            ...
                            <ItemTemplate>
                                <tr bgcolor="<%#getColor(Eval("ID").ToString())%>">
                                    ...
                                    <%-- <td height="22" width="10%">编号：
									<%#Eggsoft_Public_CL.GoodP.GetGood_Num_ID_From_Good_ID(Int32.Parse(Eval("GoodID").ToString()))%>
									</td>--%>
                                    <td height="22" width="10%">¥
									<%#Eggsoft_Public_CL.Pub.getPubMoney(Decimal.Parse(Eval("GoodPrice").ToString()))%>
                                        <a href="javascript:void(0)" title="修改金额" onclick="showMoneyFloat(<%#Eval("ID")%>,<%#Eggsoft_Public_CL.Pub.getPubMoney(Decimal.Parse(Eval("GoodPrice").ToString()))%>)">修改金额</a>
                                    </td>
                                    <td height="22" width="10%">
        ...
        <script type="text/javascript" language="javascript">
            ...
            function showFaHuoFloat(orderID, StatusNum)                    //根据屏幕的大小显示两个层 
            {
                //        $("#TxtOldMoney").val(Money);
                //        $("#TextBox_OrderID").val(orderID);
                //        $("#0").attr("checked", true);
                //      $("#RadioButtonList1").val(0);

                //$("input:radio[name='RadioButtonList1']").eq(StatusNum - 1).attr("checked", 'checked');
                $("#TextBox_OrderID").val(orderID); //借用showFloat的


                //        var range = getRange();
                //        //        $Me('doing').style.width = range.width + "px";
                //        //        $Me('doing').style.height = range.height + "px";
                //        //        $Me('doing').style.display = "block";
                document.getElementById("div_FaHuo").style.display = "";

            }


            function showPayStatusFloat(orderID, StatusNum)                    //根据屏幕的大小显示两个层 
            {
                //        $("#TxtOldMoney").val(Money);
                //        $("#TextBox_OrderID").val(orderID);
                //        $("#0").attr("checked", true);
                ...
                //        //        $Me('doing').style.height = range.height + "px";
                //        //        $Me('doing').style.display = "block";
                document.getElementById("div_PayStatus").style.display = "";

            }

            function showMoneyFloat(orderID, Money)                    //根据屏幕的大小显示两个层 
            {
                $("#TxtOldMoney").val(Money);
                $("#TextBox_OrderID").val(orderID);
                //$("#RequiredFieldValidator_TxtNewMoney_Not_Blank").attr("disabled", "");

                // s5();
                //var range = getRange();
                //$Me('divModifyMoney').style.width = range.width + "px";
                //$Me('divModifyMoney').style.height = range.height + "px";
                //$Me('divModifyMoney').style.display = "block";
                document.getElementById("divModifyMoney").style.display = "";
            ...
            function s5() {
                ...
                var popupHeight = $("#divModifyMoney").height();
                var popupWidth = $("#divModifyMoney").width();
                $("#divModifyMoney").css({
                    "position": "absolute",
                ...
                //var d5 = document.getElementById("divModifyMoney");
                //var var5Top = parseInt(document.documentElement.clientHeight / 2 + document.documentElement.scrollTop - d5.offsetHeight / 2)
                //d5.style.top = var5Top + "px";
                //var var5left = parseInt(document.documentElement.clientWidth / 2 + document.documentElement.scrollLeft - d5.offsetWidth / 2);
                //d5.style.left = var5left + "px";
            }
        ...
        <!--加一个登录层-->
        <div id="divModifyMoney" style="border: solid 10px #898989; background: #fff; padding: 10px; width: 800px; z-index: 1001; position: absolute; display: none; top: 300px; margin-left: 50%; text-align: center; left: -400px;">
            ...
            <table width="80%" border="0" style="border: 0px solid #898989; margin: auto;">
                <%--<tr>
					<td style="width: 30%;">原金额：--%>
                <tr>
                    <td style="width: 30%;">原金额：
                    </td>
                    <td>
                        <asp:TextBox ID="TextBox_OrderID" CssClass="styleVisibleFalse l_input" runat="server"> </asp:TextBox>
                        <asp:TextBox ID="TxtOldMoney" ReadOnly="true" runat="server" CssClass="l_input"> </asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td>要修改的金额： 
                    ...
                    <td>
                        <asp:TextBox ID="TxtNewMoney" runat="server" CssClass="l_input"> </asp:TextBox>
                        <asp:RegularExpressionValidator ID="RegularExpressionValidator1" runat="server" ErrorMessage="只能输入金额" ControlToValidate="TxtNewMoney" ValidationExpression="^([1-9]\d+|[0-9])(\.\d\d?)?$"></asp:RegularExpressionValidator>
                        <asp:RequiredFieldValidator ID="RequiredFieldValidator_TxtNewMoney_Not_Blank" runat="server"
                            ErrorMessage="金额不能为空!" ControlToValidate="TxtNewMoney" Display="Dynamic"
                ...
                <tr>
                    <td colspan="2" style="text-align: center;">
                        <asp:Button ID="BttLogin"
                            runat="server" Text=" 确 定 " OnClick="Btt_Money_Click" CssClass="b_input" />&nbsp; &nbsp; &nbsp; 
						<input id="BttCancel" type="button" value=" 取 消 " onclick="ShowMoneyNo()" class="b_reset" />
                    </td>
                </tr>
            </table>
        </div>
        ...
        <div id="div_PayStatus" style="border: solid 10px #898989; background: #fff; padding: 10px; width: 60%; z-index: 1001; position: absolute; display: none; top: 50%; text-align: center; left: 50%; margin: -200px 0 0 -400px;">
            <br />
            <br />
            <table width="80%" border="0" style="border: 0px solid #898989; margin: auto;">
                <tr>
                    <td style="width: 30%;">状态修改：
                    </td>
                    <td>
                        <asp:RadioButtonList ID="RadioButtonList1" runat="server"
                            ...
                            <asp:ListItem Value="3">免支付</asp:ListItem>
                            <asp:ListItem Value="4">分期支付</asp:ListItem>
                        </asp:RadioButtonList>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="2" style="text-align: center;">
                        <asp:Button ID="Button_PayWay"
                            runat="server" Text=" 确 定 " OnClick="Button_PayWay_Click" CssClass="b_input" />&nbsp; &nbsp; &nbsp; 
						<input id="Button_PayStatus_No" type="button" value=" 取 消 " onclick="Show_PayStatus_No()" class="b_reset" />
                    </td>
                </tr>
            </table>
            <br />
