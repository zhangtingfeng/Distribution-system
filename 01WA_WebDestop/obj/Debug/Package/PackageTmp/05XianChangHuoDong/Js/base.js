...
if (!Array.prototype.indexOf) {
    ...
    bjQuery(function () {
        ...
        bjQuery(".button-login").on("click",
        function () {
            var varPassWord = bjQuery("#password").val();
            if (!(varPassWord.length > 0)) {
                alert("当前活动 大屏幕 输入密码错误!!!!");
                return;
            }
            //debugger;
            aWindow.WBActivity.showLoading();


            varURL = varServiceServicesURL + "/Pub/doWeiXianChang.asmx/doverify";
            varURL += "?SceenXianChangHuoDongNumber=" + varSceenXianChangHuoDongNumber + "&ShopClientID=" + varShopClientID + "&password=" + bjQuery("#password").val();
            $.ajax({
                type: "get",
                url: varURL,
                dataType: "jsonp",
                jsonp: "jsonp", //传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名(默认为:callback)
                jsonpCallback: "jsonp201604182113Callback", //自定义的jsonp回调函数名称，默认为jQuery自动生成的随机函数名
                contentType: "application/json;charset=utf-8",//有这条报“请求格式无效：application/json;charset=utf-8。”错误，注释掉就正常
                success: function (e00000) {
                    if (e00000 && e00000.ret == 0 && e00000.data == -2) {
                        alert("活动不存在");
                        aWindow.WBActivity.hideLoading();
                        aWindow.WBActivity.showLoginForm()
                    }
                    if (e00000 && e00000.ret == 0 && e00000.data == -3) {
                        alert("当前 活动已经停止");
                        aWindow.WBActivity.hideLoading();
                        aWindow.WBActivity.showLoginForm()
                    }
                    else if (e00000 && e00000.ret == 0 && e00000.data == -4) {
                        alert("当前 活动 大屏幕 密码错误");
                        aWindow.WBActivity.hideLoading();
                        aWindow.WBActivity.showLoginForm()
                    } else if (e00000 && e00000.ret == 0 && e00000.data == 0) {
                        //debugger;
                        ///用于本地存储一个会话（session）中的数据，这些数据只有在同一个会话中的页面才能访问并且当会话结束后数据也随之销毁。因此sessionStorage不是一种持久化的本地存储，仅仅是会话级别的存储。
                        aWindow.sessionStorage.setItem("loginkeyIfNeedPassword", 1);
                        aWindow.WBActivity.hideLoginForm();
        ...
        function (lresponse) {
            ...
            if (lresponse.ret == "0") {
                if (lresponse.data && lresponse.data.parters) {
                    ...
                    if (k) {
                        var n = '<div id="' + mpartersList[0].UserID + '" class="user-item"><div class="user-img"><img src="' + mpartersList[0].avatar + '" /></div><p>' + decodeURIComponent(mpartersList[0].nick_name) + "</p></div>";
                        maxUserHeadimg++;
