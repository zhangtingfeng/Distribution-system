...
namespace Eggsoft_Public_CL
{
    public class HttpPost
    {
        ...
        private readonly Hashtable postingData = new Hashtable();
        private string url;
        ...
        public HttpPost(string url, HttpClientContext context)
        ...
        public HttpPost(string url, HttpClientContext context, bool keepContext)
        {
            this.url = url;
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            //if (context.Referer != null && !context.Referer.Equals(""))
            //{
            //    req.Referer = context.Referer;
            //}
            ...
            if (verb == HttpVerb.POST)
            {
                ...
                else
                {
                    ...
                    foreach (string key in postingData.Keys)
                    {
                        sb.AppendFormat("{0}={1}&", HttpUtility.UrlEncode(key, Encoding.GetEncoding("GBK")), HttpUtility.UrlEncode(postingData[key].ToString(), Encoding.GetEncoding("GBK")));
