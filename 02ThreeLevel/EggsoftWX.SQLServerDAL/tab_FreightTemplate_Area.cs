...
{
    /// <summary>
    /// 数据访问类tab_FreightTemplate_Area。
    /// </summary>
    public class tab_FreightTemplate_Area:Itab_FreightTemplate_Area
        {
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [tab_FreightTemplate_Area]");
       ...
       /// <summary>
		/// 增加一条数据 model
		/// </summary>
		public Int32 Add(EggsoftWX.Model.tab_FreightTemplate_Area model)
		{
			IDataParameter[] iData = new SqlParameter[2];
			iData[0] = new SqlParameter("@TableName", "tab_FreightTemplate_Area");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [tab_FreightTemplate_Area]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [tab_FreightTemplate_Area] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [tab_FreightTemplate_Area] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       /// <summary>
		...
		public Int32 ExistsCount(string strWhere)
		{
           strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [tab_FreightTemplate_Area] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
			if((Object.Equals(obj,null))||(Object.Equals(obj,System.DBNull.Value)))
			{
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [tab_FreightTemplate_Area] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [tab_FreightTemplate_Area] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [tab_FreightTemplate_Area] where 1>0 " + strWhere);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [tab_FreightTemplate_Area] where 1>0 " + strWhere);
            return DbHelperSQL.GetList(strSql.ToString());
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [tab_FreightTemplate_Area] where 1>0 " + strWhere);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model.tab_FreightTemplate_Area GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [tab_FreightTemplate_Area] ");
			strSql.Append(" where "+strWhere);
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["FreightMore"].ToString()!="")
				{
					model.FreightMore=decimal.Parse(ds.Tables[0].Rows[0]["FreightMore"].ToString());
				}
				...
				model.AreaList=ds.Tables[0].Rows[0]["AreaList"].ToString();
				if(ds.Tables[0].Rows[0]["FreightTemplate_ID"].ToString()!="")
				{
					model.FreightTemplate_ID=Int32.Parse(ds.Tables[0].Rows[0]["FreightTemplate_ID"].ToString());
				}
				if(ds.Tables[0].Rows[0]["HowmanysNoFreight"].ToString()!="")
				{
					model.HowmanysNoFreight=Int32.Parse(ds.Tables[0].Rows[0]["HowmanysNoFreight"].ToString());
				}
				if(ds.Tables[0].Rows[0]["HowmuchNoFreight"].ToString()!="")
				{
					model.HowmuchNoFreight=decimal.Parse(ds.Tables[0].Rows[0]["HowmuchNoFreight"].ToString());
				}
				if(ds.Tables[0].Rows[0]["HowkgNoFreight"].ToString()!="")
				{
					model.HowkgNoFreight=decimal.Parse(ds.Tables[0].Rows[0]["HowkgNoFreight"].ToString());
				}
				return model;
			}
			else
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model.tab_FreightTemplate_Area GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [tab_FreightTemplate_Area] ");
				strSql.Append(" where "+strWhere);
				DbHelperSQL.ExecuteSql(strSql.ToString());
		}
       #endregion
       ...
       #region update strSet strWhere 更新n条数据 更新n条数据 memo 0017
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_FreightTemplate_Area] set ");
			strSql.Append(strSet);
			strSql.Append( " where " );
			strSql.Append(strWhere);
			DbHelperSQL.ExecuteSql(strSql.ToString());
       ...
       #region model update 更新一条数据 memo 0018
		...
		/// 更新一条数据
		/// </summary>
		public void Update(EggsoftWX.Model.tab_FreightTemplate_Area model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_FreightTemplate_Area] set ");
			strSql.Append("[Name]='" + model.Name + "',");
			strSql.Append("[ShopClient_ID]=" + model.ShopClient_ID + ",");
			strSql.Append("[Freight]=" + model.Freight + ",");
			strSql.Append("[FreightMore]=" + model.FreightMore + ",");
			strSql.Append("[UpdateTime]='" + model.UpdateTime.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',");
			strSql.Append("[CreateTime]='" + model.CreateTime.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',");
			strSql.Append("[Remarks]='" + model.Remarks + "',");
			strSql.Append("[AreaList]='" + model.AreaList + "',");
			strSql.Append("[FreightTemplate_ID]=" + model.FreightTemplate_ID + ",");
			strSql.Append("[HowmanysNoFreight]=" + model.HowmanysNoFreight + ",");
			strSql.Append("[HowmuchNoFreight]=" + model.HowmuchNoFreight + ",");
			strSql.Append("[HowkgNoFreight]=" + model.HowkgNoFreight + "");
        ...
        public DataTable GetList(string topNum, string fields, string strWhere)
       {
           ...
           strSql.Append(" FROM   [tab_FreightTemplate_Area]  where 1 > 0 " + strWhere);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc)
        {
           ...
           if ((strConditions.IndexOf("and") == -1) && (strConditions!="")) strConditions = "and " + strConditions;
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[tab_FreightTemplate_Area]", strConditions, orderField, isDesc);
        }
