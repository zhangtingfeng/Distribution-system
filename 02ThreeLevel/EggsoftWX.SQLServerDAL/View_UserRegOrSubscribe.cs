...
{
    /// <summary>
    /// 数据访问类View_UserRegOrSubscribe。
    /// </summary>
    public class View_UserRegOrSubscribe:IView_UserRegOrSubscribe
        {
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [View_UserRegOrSubscribe]");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [View_UserRegOrSubscribe]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [View_UserRegOrSubscribe] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [View_UserRegOrSubscribe] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       /// <summary>
		...
		public Int32 ExistsCount(string strWhere)
		{
           strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [View_UserRegOrSubscribe] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
			if((Object.Equals(obj,null))||(Object.Equals(obj,System.DBNull.Value)))
			{
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [View_UserRegOrSubscribe] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [View_UserRegOrSubscribe] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [View_UserRegOrSubscribe] where 1>0 " + strWhere);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [View_UserRegOrSubscribe] where 1>0 " + strWhere);
            return DbHelperSQL.GetList(strSql.ToString());
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [View_UserRegOrSubscribe] where 1>0 " + strWhere);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model.View_UserRegOrSubscribe GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [View_UserRegOrSubscribe] ");
			strSql.Append(" where "+strWhere);
			EggsoftWX.Model.View_UserRegOrSubscribe model=new EggsoftWX.Model.View_UserRegOrSubscribe();
			DataSet ds=DbHelperSQL.Query(strSql.ToString());
			if(ds.Tables[0].Rows.Count>0)
			{
				model.Updatetime=ds.Tables[0].Rows[0]["Updatetime"].ToString();
				if(ds.Tables[0].Rows[0]["RegCount"].ToString()!="")
				{
					model.RegCount=Int32.Parse(ds.Tables[0].Rows[0]["RegCount"].ToString());
				}
				if(ds.Tables[0].Rows[0]["SubCount"].ToString()!="")
				{
					model.SubCount=Int32.Parse(ds.Tables[0].Rows[0]["SubCount"].ToString());
				}
				return model;
			}
			else
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model.View_UserRegOrSubscribe GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [View_UserRegOrSubscribe] ");
				strSql.Append(" where "+strWhere);
				DbHelperSQL.ExecuteSql(strSql.ToString());
		}
       #endregion
        ...
        public DataTable GetList(string topNum, string fields, string strWhere)
       {
           ...
           strSql.Append(" FROM   [View_UserRegOrSubscribe]  where 1 > 0 " + strWhere);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc)
        {
           ...
           if ((strConditions.IndexOf("and") == -1) && (strConditions!="")) strConditions = "and " + strConditions;
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[View_UserRegOrSubscribe]", strConditions, orderField, isDesc);
        }
