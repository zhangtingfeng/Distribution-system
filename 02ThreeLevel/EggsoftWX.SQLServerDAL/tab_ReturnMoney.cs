...
{
    /// <summary>
    /// 数据访问类tab_ReturnMoney。
    /// </summary>
    public class tab_ReturnMoney:Itab_ReturnMoney
        {
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [tab_ReturnMoney]");
       ...
       /// <summary>
		/// 增加一条数据 model
		/// </summary>
		public Int32 Add(EggsoftWX.Model.tab_ReturnMoney model)
		{
			IDataParameter[] iData = new SqlParameter[2];
			iData[0] = new SqlParameter("@TableName", "tab_ReturnMoney");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [tab_ReturnMoney]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [tab_ReturnMoney] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [tab_ReturnMoney] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       /// <summary>
		...
		public Int32 ExistsCount(string strWhere)
		{
           strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [tab_ReturnMoney] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
			if((Object.Equals(obj,null))||(Object.Equals(obj,System.DBNull.Value)))
			{
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [tab_ReturnMoney] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [tab_ReturnMoney] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [tab_ReturnMoney] where 1>0 " + strWhere);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [tab_ReturnMoney] where 1>0 " + strWhere);
            return DbHelperSQL.GetList(strSql.ToString());
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [tab_ReturnMoney] where 1>0 " + strWhere);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model.tab_ReturnMoney GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [tab_ReturnMoney] ");
			strSql.Append(" where "+strWhere);
			EggsoftWX.Model.tab_ReturnMoney model=new EggsoftWX.Model.tab_ReturnMoney();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["RefundMoney"].ToString()!="")
				{
					model.RefundMoney=decimal.Parse(ds.Tables[0].Rows[0]["RefundMoney"].ToString());
				}
				if(ds.Tables[0].Rows[0]["AdminFinance_User"].ToString()!="")
				{
					model.AdminFinance_User=Int32.Parse(ds.Tables[0].Rows[0]["AdminFinance_User"].ToString());
				}
				if(ds.Tables[0].Rows[0]["FinanceCheck"].ToString()!="")
				{
					model.FinanceCheck=Convert.ToBoolean(ds.Tables[0].Rows[0]["FinanceCheck"].ToString());
				}
				if(ds.Tables[0].Rows[0]["AdminCheck"].ToString()!="")
				{
					model.AdminCheck=Convert.ToBoolean(ds.Tables[0].Rows[0]["AdminCheck"].ToString());
				}
				if(ds.Tables[0].Rows[0]["ShopClientID"].ToString()!="")
				{
					model.ShopClientID=Int32.Parse(ds.Tables[0].Rows[0]["ShopClientID"].ToString());
				}
				model.ReturnMoneyTitle=ds.Tables[0].Rows[0]["ReturnMoneyTitle"].ToString();
				model.ReturnMoneyContent=ds.Tables[0].Rows[0]["ReturnMoneyContent"].ToString();
				return model;
			}
			else
			{
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model.tab_ReturnMoney GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [tab_ReturnMoney] ");
				strSql.Append(" where "+strWhere);
				DbHelperSQL.ExecuteSql(strSql.ToString());
		}
       #endregion
       ...
       #region update strSet strWhere 更新n条数据 更新n条数据 memo 0017
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_ReturnMoney] set ");
			strSql.Append(strSet);
			strSql.Append( " where " );
			strSql.Append(strWhere);
			DbHelperSQL.ExecuteSql(strSql.ToString());
       ...
       #region model update 更新一条数据 memo 0018
		...
		/// 更新一条数据
		/// </summary>
		public void Update(EggsoftWX.Model.tab_ReturnMoney model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_ReturnMoney] set ");
			strSql.Append("[UpdateTime]='" + model.UpdateTime.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',");
			strSql.Append("[OrderID]=" + model.OrderID + ",");
			strSql.Append("[RefundMoney]=" + model.RefundMoney + ",");
			strSql.Append("[AdminFinance_User]=" + model.AdminFinance_User + ",");
			strSql.Append("[FinanceCheck]=" + (model.FinanceCheck? Convert.ToInt32(1) : Convert.ToInt32(0)) + ",");
			strSql.Append("[AdminCheck]=" + (model.AdminCheck? Convert.ToInt32(1) : Convert.ToInt32(0)) + ",");
			strSql.Append("[ShopClientID]=" + model.ShopClientID + ",");
			strSql.Append("[ReturnMoneyTitle]='" + model.ReturnMoneyTitle + "',");
			strSql.Append("[ReturnMoneyContent]='" + model.ReturnMoneyContent + "'");
        ...
        public DataTable GetList(string topNum, string fields, string strWhere)
       {
           ...
           strSql.Append(" FROM   [tab_ReturnMoney]  where 1 > 0 " + strWhere);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc)
        {
           ...
           if ((strConditions.IndexOf("and") == -1) && (strConditions!="")) strConditions = "and " + strConditions;
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[tab_ReturnMoney]", strConditions, orderField, isDesc);
        }
