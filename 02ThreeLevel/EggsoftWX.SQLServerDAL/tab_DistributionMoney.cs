...
{
    /// <summary>
    /// 数据访问类tab_DistributionMoney。
    /// </summary>
    public class tab_DistributionMoney:Itab_DistributionMoney
        {
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [tab_DistributionMoney]");
       ...
       /// <summary>
		/// 增加一条数据 model
		/// </summary>
		public Int32 Add(EggsoftWX.Model.tab_DistributionMoney model)
		{
			IDataParameter[] iData = new SqlParameter[2];
			iData[0] = new SqlParameter("@TableName", "tab_DistributionMoney");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [tab_DistributionMoney]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [tab_DistributionMoney] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			int cmdresult;
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [tab_DistributionMoney] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       /// <summary>
		...
		public Int32 ExistsCount(string strWhere)
		{
           strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [tab_DistributionMoney] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			int cmdresult;
			if((Object.Equals(obj,null))||(Object.Equals(obj,System.DBNull.Value)))
			{
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [tab_DistributionMoney] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [tab_DistributionMoney] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [tab_DistributionMoney] where 1>0 " + strWhere);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [tab_DistributionMoney] where 1>0 " + strWhere);
            return DbHelperSQL.GetList(strSql.ToString());
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [tab_DistributionMoney] where 1>0 " + strWhere);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model.tab_DistributionMoney GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [tab_DistributionMoney] ");
			strSql.Append(" where "+strWhere);
			EggsoftWX.Model.tab_DistributionMoney model=new EggsoftWX.Model.tab_DistributionMoney();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["Partner"].ToString()!="")
				{
					model.Partner=decimal.Parse(ds.Tables[0].Rows[0]["Partner"].ToString());
				}
				if(ds.Tables[0].Rows[0]["GreatParent"].ToString()!="")
				{
					model.GreatParent=decimal.Parse(ds.Tables[0].Rows[0]["GreatParent"].ToString());
				}
				if(ds.Tables[0].Rows[0]["GrandParent"].ToString()!="")
				{
					model.GrandParent=decimal.Parse(ds.Tables[0].Rows[0]["GrandParent"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Parent"].ToString()!="")
				{
					model.Parent=decimal.Parse(ds.Tables[0].Rows[0]["Parent"].ToString());
				}
				...
				model.OperateMan=ds.Tables[0].Rows[0]["OperateMan"].ToString();
				if(ds.Tables[0].Rows[0]["ShopGet"].ToString()!="")
				{
					model.ShopGet=decimal.Parse(ds.Tables[0].Rows[0]["ShopGet"].ToString());
				}
				if(ds.Tables[0].Rows[0]["WeiBaiGet"].ToString()!="")
				{
					model.WeiBaiGet=decimal.Parse(ds.Tables[0].Rows[0]["WeiBaiGet"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Recommended"].ToString()!="")
				{
					model.Recommended=decimal.Parse(ds.Tables[0].Rows[0]["Recommended"].ToString());
				}
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model.tab_DistributionMoney GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [tab_DistributionMoney] ");
				strSql.Append(" where "+strWhere);
				DbHelperSQL.ExecuteSql(strSql.ToString());
		}
       #endregion
       ...
       #region update strSet strWhere 更新n条数据 更新n条数据 memo 0017
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_DistributionMoney] set ");
			strSql.Append(strSet);
			strSql.Append( " where " );
			strSql.Append(strWhere);
			DbHelperSQL.ExecuteSql(strSql.ToString());
       ...
       #region model update 更新一条数据 memo 0018
		...
		/// 更新一条数据
		/// </summary>
		public void Update(EggsoftWX.Model.tab_DistributionMoney model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_DistributionMoney] set ");
			if (model.ID != null) strSql.Append("[ID]=@ID,");
			if (model.Partner != null) strSql.Append("[Partner]=@Partner,");
			if (model.GreatParent != null) strSql.Append("[GreatParent]=@GreatParent,");
			if (model.GrandParent != null) strSql.Append("[GrandParent]=@GrandParent,");
			if (model.Parent != null) strSql.Append("[Parent]=@Parent,");
			...
			if (String.IsNullOrEmpty(model.OperateMan) == false) strSql.Append("[OperateMan]=@OperateMan,");
			if (model.ShopGet != null) strSql.Append("[ShopGet]=@ShopGet,");
			if (model.WeiBaiGet != null) strSql.Append("[WeiBaiGet]=@WeiBaiGet,");
			if (model.Recommended != null) strSql.Append("[Recommended]=@Recommended,");
			...
			if (model.Partner != null) ParameterToArrayList.Add(new SqlParameter("@Partner",model.Partner));
			if (model.GreatParent != null) ParameterToArrayList.Add(new SqlParameter("@GreatParent",model.GreatParent));
			if (model.GrandParent != null) ParameterToArrayList.Add(new SqlParameter("@GrandParent",model.GrandParent));
			if (model.Parent != null) ParameterToArrayList.Add(new SqlParameter("@Parent",model.Parent));
			...
			if (String.IsNullOrEmpty(model.OperateMan) == false) ParameterToArrayList.Add(new SqlParameter("@OperateMan",model.OperateMan));
			if (model.ShopGet != null) ParameterToArrayList.Add(new SqlParameter("@ShopGet",model.ShopGet));
			if (model.WeiBaiGet != null) ParameterToArrayList.Add(new SqlParameter("@WeiBaiGet",model.WeiBaiGet));
			if (model.Recommended != null) ParameterToArrayList.Add(new SqlParameter("@Recommended",model.Recommended));
        ...
        public DataTable GetList(string topNum, string fields, string strWhere)
       {
           ...
           strSql.Append(" FROM   [tab_DistributionMoney]  where 1 > 0 " + strWhere);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc)
        {
           ...
           if ((strConditions.IndexOf("and") == -1) && (strConditions!="")) strConditions = "and " + strConditions;
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[tab_DistributionMoney]", strConditions, orderField, isDesc);
        }
