...
{
    /// <summary>
    /// 数据访问类AccountsRelation。
    /// </summary>
    public class AccountsRelation:IAccountsRelation
        {
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [AccountsRelation]");
       ...
       /// <summary>
		/// 增加一条数据 model
		/// </summary>
		public Int32 Add(EggsoftWX.Model.AccountsRelation model)
		{
			IDataParameter[] iData = new SqlParameter[2];
			iData[0] = new SqlParameter("@TableName", "AccountsRelation");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [AccountsRelation]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [AccountsRelation] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [AccountsRelation] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       /// <summary>
		...
		public Int32 ExistsCount(string strWhere)
		{
           strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [AccountsRelation] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
			if((Object.Equals(obj,null))||(Object.Equals(obj,System.DBNull.Value)))
			{
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [AccountsRelation] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [AccountsRelation] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [AccountsRelation] where 1>0 " + strWhere);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [AccountsRelation] where 1>0 " + strWhere);
            return DbHelperSQL.GetList(strSql.ToString());
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [AccountsRelation] where 1>0 " + strWhere);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model.AccountsRelation GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [AccountsRelation] ");
			strSql.Append(" where "+strWhere);
			EggsoftWX.Model.AccountsRelation model=new EggsoftWX.Model.AccountsRelation();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				{
					model.Uid=Int32.Parse(ds.Tables[0].Rows[0]["Uid"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Pid"].ToString()!="")
				{
					model.Pid=Int32.Parse(ds.Tables[0].Rows[0]["Pid"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Wid"].ToString()!="")
				{
					model.Wid=Int32.Parse(ds.Tables[0].Rows[0]["Wid"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Yid"].ToString()!="")
				{
					model.Yid=Int32.Parse(ds.Tables[0].Rows[0]["Yid"].ToString());
				}
				return model;
			}
			else
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model.AccountsRelation GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [AccountsRelation] ");
				strSql.Append(" where "+strWhere);
				DbHelperSQL.ExecuteSql(strSql.ToString());
		}
       #endregion
       ...
       #region update strSet strWhere 更新n条数据 更新n条数据 memo 0017
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [AccountsRelation] set ");
			strSql.Append(strSet);
			strSql.Append( " where " );
			strSql.Append(strWhere);
			DbHelperSQL.ExecuteSql(strSql.ToString());
       ...
       #region model update 更新一条数据 memo 0018
		...
		/// 更新一条数据
		/// </summary>
		public void Update(EggsoftWX.Model.AccountsRelation model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [AccountsRelation] set ");
			strSql.Append("[Uid]=" + model.Uid + ",");
			strSql.Append("[Pid]=" + model.Pid + ",");
			strSql.Append("[Wid]=" + model.Wid + ",");
			strSql.Append("[Yid]=" + model.Yid + "");
        ...
        public DataTable GetList(string topNum, string fields, string strWhere)
       {
           ...
           strSql.Append(" FROM   [AccountsRelation]  where 1 > 0 " + strWhere);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc)
        {
           ...
           if ((strConditions.IndexOf("and") == -1) && (strConditions!="")) strConditions = "and " + strConditions;
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[AccountsRelation]", strConditions, orderField, isDesc);
        }
