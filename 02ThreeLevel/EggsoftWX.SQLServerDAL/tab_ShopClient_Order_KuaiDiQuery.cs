...
{
    /// <summary>
    /// 数据访问类tab_ShopClient_Order_KuaiDiQuery。
    /// </summary>
    public class tab_ShopClient_Order_KuaiDiQuery:Itab_ShopClient_Order_KuaiDiQuery
        {
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [tab_ShopClient_Order_KuaiDiQuery]");
       ...
       /// <summary>
		/// 增加一条数据 model
		/// </summary>
		public Int32 Add(EggsoftWX.Model.tab_ShopClient_Order_KuaiDiQuery model)
		{
			IDataParameter[] iData = new SqlParameter[2];
			iData[0] = new SqlParameter("@TableName", "tab_ShopClient_Order_KuaiDiQuery");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [tab_ShopClient_Order_KuaiDiQuery]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [tab_ShopClient_Order_KuaiDiQuery] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [tab_ShopClient_Order_KuaiDiQuery] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       /// <summary>
		...
		public Int32 ExistsCount(string strWhere)
		{
           strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(*) from [tab_ShopClient_Order_KuaiDiQuery] where 1>0 "+strWhere+" ");
			object obj=DbHelperSQL.GetSingle(strSql.ToString());
			Int32 cmdresult;
			if((Object.Equals(obj,null))||(Object.Equals(obj,System.DBNull.Value)))
			{
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [tab_ShopClient_Order_KuaiDiQuery] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [tab_ShopClient_Order_KuaiDiQuery] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [tab_ShopClient_Order_KuaiDiQuery] where 1>0 " + strWhere);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [tab_ShopClient_Order_KuaiDiQuery] where 1>0 " + strWhere);
            return DbHelperSQL.GetList(strSql.ToString());
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [tab_ShopClient_Order_KuaiDiQuery] where 1>0 " + strWhere);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model.tab_ShopClient_Order_KuaiDiQuery GetModel(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [tab_ShopClient_Order_KuaiDiQuery] ");
			strSql.Append(" where "+strWhere);
			EggsoftWX.Model.tab_ShopClient_Order_KuaiDiQuery model=new EggsoftWX.Model.tab_ShopClient_Order_KuaiDiQuery();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				model.KuaiDiCompany=ds.Tables[0].Rows[0]["KuaiDiCompany"].ToString();
				if(ds.Tables[0].Rows[0]["QueryCount"].ToString()!="")
				{
					model.QueryCount=Int32.Parse(ds.Tables[0].Rows[0]["QueryCount"].ToString());
				}
				model.KuaiDiNumber=ds.Tables[0].Rows[0]["KuaiDiNumber"].ToString();
				model.JuHeResultList=ds.Tables[0].Rows[0]["JuHeResultList"].ToString();
				model.FreeXML=ds.Tables[0].Rows[0]["FreeXML"].ToString();
				return model;
			}
			else
			{
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model.tab_ShopClient_Order_KuaiDiQuery GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [tab_ShopClient_Order_KuaiDiQuery] ");
				strSql.Append(" where "+strWhere);
				DbHelperSQL.ExecuteSql(strSql.ToString());
		}
       #endregion
       ...
       #region update strSet strWhere 更新n条数据 更新n条数据 memo 0017
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_ShopClient_Order_KuaiDiQuery] set ");
			strSql.Append(strSet);
			strSql.Append( " where " );
			strSql.Append(strWhere);
			DbHelperSQL.ExecuteSql(strSql.ToString());
       ...
       #region model update 更新一条数据 memo 0018
		...
		/// 更新一条数据
		/// </summary>
		public void Update(EggsoftWX.Model.tab_ShopClient_Order_KuaiDiQuery model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [tab_ShopClient_Order_KuaiDiQuery] set ");
			...
			if (model.UpdateTime != DateTime.MinValue) strSql.Append("[UpdateTime]=@UpdateTime,");
			if (model.InsertTime != DateTime.MinValue) strSql.Append("[InsertTime]=@InsertTime,");
			if (model.UserID != null) strSql.Append("[UserID]=@UserID,");
			if (String.IsNullOrEmpty(model.KuaiDiCompany) == false) strSql.Append("[KuaiDiCompany]=@KuaiDiCompany,");
			if (model.QueryCount != null) strSql.Append("[QueryCount]=@QueryCount,");
			if (String.IsNullOrEmpty(model.KuaiDiNumber) == false) strSql.Append("[KuaiDiNumber]=@KuaiDiNumber,");
			if (String.IsNullOrEmpty(model.JuHeResultList) == false) strSql.Append("[JuHeResultList]=@JuHeResultList,");
			if (String.IsNullOrEmpty(model.FreeXML) == false) strSql.Append("[FreeXML]=@FreeXML,");
			...
			if (model.UpdateTime != DateTime.MinValue) ParameterToArrayList.Add(new SqlParameter("@UpdateTime",model.UpdateTime));
			if (model.InsertTime != DateTime.MinValue) ParameterToArrayList.Add(new SqlParameter("@InsertTime",model.InsertTime));
			if (model.UserID != null) ParameterToArrayList.Add(new SqlParameter("@UserID",model.UserID));
			if (String.IsNullOrEmpty(model.KuaiDiCompany) == false) ParameterToArrayList.Add(new SqlParameter("@KuaiDiCompany",model.KuaiDiCompany));
			if (model.QueryCount != null) ParameterToArrayList.Add(new SqlParameter("@QueryCount",model.QueryCount));
			if (String.IsNullOrEmpty(model.KuaiDiNumber) == false) ParameterToArrayList.Add(new SqlParameter("@KuaiDiNumber",model.KuaiDiNumber));
			if (String.IsNullOrEmpty(model.JuHeResultList) == false) ParameterToArrayList.Add(new SqlParameter("@JuHeResultList",model.JuHeResultList));
			if (String.IsNullOrEmpty(model.FreeXML) == false) ParameterToArrayList.Add(new SqlParameter("@FreeXML",model.FreeXML));
        ...
        public DataTable GetList(string topNum, string fields, string strWhere)
       {
           ...
           strSql.Append(" FROM   [tab_ShopClient_Order_KuaiDiQuery]  where 1 > 0 " + strWhere);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc)
        {
           ...
           if ((strConditions.IndexOf("and") == -1) && (strConditions!="")) strConditions = "and " + strConditions;
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[tab_ShopClient_Order_KuaiDiQuery]", strConditions, orderField, isDesc);
        }
