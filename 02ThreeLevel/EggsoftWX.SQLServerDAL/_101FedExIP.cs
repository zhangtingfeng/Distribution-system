...
namespace EggsoftWX.SQLServerDAL
{
    ...
    /// 数据访问类_101FedExIP。
    /// </summary>
    public class _101FedExIP:I_101FedExIP
        {
        public _101FedExIP()
        {}
        #region  成员方法
       ...
       /// <summary>
		...
		public Int32 GetMaxId()
		{
			...
			strSql.Append("select max(ID)+1 from [_101FedExIP]");
       ...
       /// <summary>
		/// 增加一条数据 model
		/// </summary>
		public Int32 Add(EggsoftWX.Model._101FedExIP model)
		{
			IDataParameter[] iData = new SqlParameter[2];
			iData[0] = new SqlParameter("@TableName", "_101FedExIP");
       ...
       /// <summary>
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into [_101FedExIP]");
       ...
       /// <summary>
		...
		public bool Exists(string strWhere, params object[] objs)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from [_101FedExIP] where 1>0 "+strWhere+" ");
			SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
       ...
       /// <summary>
		...
		public bool Exists(Int32 ID)
		{
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [_101FedExIP] where ID=" + ID + "");
            object obj = DbHelperSQL.GetSingle(strSql.ToString());
            Int32 cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
       ...
       #region 获得数据列表GetList1 string strWhere memo 0008
		...
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from [_101FedExIP] ");
       ...
       #region 获得指定数据列表GetList2 string strItem,string strWhere memo 0009
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select " + strItem + " from [_101FedExIP] ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
       ...
       #region IList<string> GetFieldValues  memo 0010
            ...
            public IList<string> GetFieldValues(string fields, string strWhere, params object[] objs)
            {
                StringBuilder strSql = new StringBuilder();
                strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
                strSql.Append("select " + fields + " from [_101FedExIP] where 1>0 " + strWhere);
			SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
       ...
       #region IList<string> GetFieldValues(string topNum, string fields, string strWhere) memo 0011
        ...
        public IList<string> GetFieldValues(string topNum, string fields, string strWhere, params object[] objs)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + topNum + " " + fields + " from [_101FedExIP] where 1>0 " + strWhere);
			SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
            return DbHelperSQL.GetList(strSql.ToString(),ParameterToList);
        }
       #endregion
       ...
       #region object Scalar(string field, string strWhere) memo 0012
        ...
        public object Scalar(string field, string strWhere, params object[] objs)
        {
            strWhere = strWhere.Trim().ToLower(); if (strWhere != "") if (strWhere.IndexOf("and") != 0) strWhere = "and " + strWhere;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select " + field + " from [_101FedExIP] where 1>0 " + strWhere);
			SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
       ...
       #region 得到一个对象实体 strWhere memo 0013
		...
		/// 得到一个对象实体  strWhere
		/// </summary>
		public EggsoftWX.Model._101FedExIP GetModel(string strWhere, params object[] objs)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from [_101FedExIP] ");
			strSql.Append(" where "+strWhere);
			EggsoftWX.Model._101FedExIP model=new EggsoftWX.Model._101FedExIP();
			SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				model.Kgs=ds.Tables[0].Rows[0]["Kgs"].ToString();
				if(ds.Tables[0].Rows[0]["_1"].ToString()!="")
				{
					model._1=float.Parse(ds.Tables[0].Rows[0]["_1"].ToString());
				}
				if(ds.Tables[0].Rows[0]["_2"].ToString()!="")
				{
					model._2=float.Parse(ds.Tables[0].Rows[0]["_2"].ToString());
				}
				if(ds.Tables[0].Rows[0]["A"].ToString()!="")
				{
					model.A=float.Parse(ds.Tables[0].Rows[0]["A"].ToString());
				}
				if(ds.Tables[0].Rows[0]["B"].ToString()!="")
				{
					model.B=float.Parse(ds.Tables[0].Rows[0]["B"].ToString());
				}
				if(ds.Tables[0].Rows[0]["C"].ToString()!="")
				{
					model.C=float.Parse(ds.Tables[0].Rows[0]["C"].ToString());
				}
				if(ds.Tables[0].Rows[0]["D"].ToString()!="")
				{
					model.D=float.Parse(ds.Tables[0].Rows[0]["D"].ToString());
				}
				if(ds.Tables[0].Rows[0]["E"].ToString()!="")
				{
					model.E=float.Parse(ds.Tables[0].Rows[0]["E"].ToString());
				}
				if(ds.Tables[0].Rows[0]["F"].ToString()!="")
				{
					model.F=float.Parse(ds.Tables[0].Rows[0]["F"].ToString());
				}
				if(ds.Tables[0].Rows[0]["G"].ToString()!="")
				{
					model.G=float.Parse(ds.Tables[0].Rows[0]["G"].ToString());
				}
				if(ds.Tables[0].Rows[0]["H"].ToString()!="")
				{
					model.H=float.Parse(ds.Tables[0].Rows[0]["H"].ToString());
				}
				if(ds.Tables[0].Rows[0]["M"].ToString()!="")
				{
					model.M=float.Parse(ds.Tables[0].Rows[0]["M"].ToString());
				}
				if(ds.Tables[0].Rows[0]["N"].ToString()!="")
				{
					model.N=float.Parse(ds.Tables[0].Rows[0]["N"].ToString());
				}
				if(ds.Tables[0].Rows[0]["O"].ToString()!="")
				{
					model.O=float.Parse(ds.Tables[0].Rows[0]["O"].ToString());
				}
				if(ds.Tables[0].Rows[0]["P"].ToString()!="")
				{
					model.P=float.Parse(ds.Tables[0].Rows[0]["P"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Q"].ToString()!="")
				{
					model.Q=float.Parse(ds.Tables[0].Rows[0]["Q"].ToString());
				}
				if(ds.Tables[0].Rows[0]["R"].ToString()!="")
				{
					model.R=float.Parse(ds.Tables[0].Rows[0]["R"].ToString());
				}
				if(ds.Tables[0].Rows[0]["S"].ToString()!="")
				{
					model.S=float.Parse(ds.Tables[0].Rows[0]["S"].ToString());
				}
				if(ds.Tables[0].Rows[0]["T"].ToString()!="")
				{
					model.T=float.Parse(ds.Tables[0].Rows[0]["T"].ToString());
				}
				if(ds.Tables[0].Rows[0]["U"].ToString()!="")
				{
					model.U=float.Parse(ds.Tables[0].Rows[0]["U"].ToString());
				}
				if(ds.Tables[0].Rows[0]["V"].ToString()!="")
				{
					model.V=float.Parse(ds.Tables[0].Rows[0]["V"].ToString());
				}
				if(ds.Tables[0].Rows[0]["X"].ToString()!="")
				{
					model.X=float.Parse(ds.Tables[0].Rows[0]["X"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Y"].ToString()!="")
				{
					model.Y=float.Parse(ds.Tables[0].Rows[0]["Y"].ToString());
				}
				if(ds.Tables[0].Rows[0]["Z"].ToString()!="")
				{
					model.Z=float.Parse(ds.Tables[0].Rows[0]["Z"].ToString());
				}
       ...
       #region 得到一个对象实体 ID  memo 0014
		...
		/// 得到一个对象实体
		/// </summary>
		public EggsoftWX.Model._101FedExIP GetModel(Int32 ID)
		{
       ...
       /// <summary>
		...
		{
				StringBuilder strSql=new StringBuilder();
				strSql.Append("delete from [_101FedExIP] ");
				strSql.Append(" where "+strWhere);
				SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
       ...
       #region update strSet strWhere 更新n条数据 更新n条数据 memo 0017
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [_101FedExIP] set ");
			strSql.Append(strSet);
			strSql.Append( " where " );
			strSql.Append(strWhere);
				SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
       ...
       #region model update 更新一条数据 memo 0018
		...
		/// 更新一条数据
		/// </summary>
		public int Update(EggsoftWX.Model._101FedExIP model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update [_101FedExIP] set ");
			if (model.ID != null) strSql.Append("[ID]=@ID,");
			if (String.IsNullOrEmpty(model.Type) == false) strSql.Append("[Type]=@Type,");
			if (String.IsNullOrEmpty(model.Kgs) == false) strSql.Append("[Kgs]=@Kgs,");
			...
			if (model.C != null) strSql.Append("[C]=@C,");
			...
			if (String.IsNullOrEmpty(model.Kgs) == false) ParameterToArrayList.Add(new SqlParameter("@Kgs",model.Kgs));
			...
			if (model.C != null) ParameterToArrayList.Add(new SqlParameter("@C",model.C));
        ...
        public DataTable GetList(string topNum, string fields, string strWhere, params object[] objs)
       {
           ...
           strSql.Append(" FROM   [_101FedExIP]  where 1 > 0 " + strWhere);
           SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(strSql.ToString(), objs);
       ...
       #region 大数据量快速分页,50万以上数据分页 memo 0020
        ...
        public DataTable GetPageDataTable(Int32 pageIndex, Int32 pageSize, string fields, string strConditions, string orderField, bool isDesc, params object[] objs)
        {
           ...
           SqlParameter[] ParameterToList = EggsoftWX.SQLServerDAL.Util.ChangeParamsObjectToSqlParameter.ChangeParamsObjectToSqlParameterAction(fields+" "+strConditions+" "+orderField, objs);
            return DbHelperSQL.GetPageDataTable(pageIndex, pageSize, fields, "[_101FedExIP]", strConditions, orderField, isDesc,ParameterToList);
        }
