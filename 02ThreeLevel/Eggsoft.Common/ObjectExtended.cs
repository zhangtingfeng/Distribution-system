...
namespace Eggsoft.Common
{
    public static class ObjectExtended
    {
        ...
        /// </summary>
        public static dynamic XMLtoJsonDynamicObject(this String val)
        {
            try
            {
                if (val == "") return null;
        ...
        /// Json.net实现方便的Json转C#（dynamic动态类型）对象
        /// </summary>
        public static dynamic toJsonDynamicObject(this String val)
        {
            try
            {
                if (val == "") return "";
                return Newtonsoft.Json.JsonConvert.DeserializeObject<dynamic>(val);
            }
            catch (Exception ex)
            {
                //LogController.writeErrorLog(ex, "ObjectExtended toJson");
