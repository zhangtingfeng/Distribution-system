...
var popUpCal = {
    ...
    prevText: '&lt;前一月', // Display text for previous month link
    nextText: '后一月&gt;', // Display text for next month link
    ...
    // appears on a button
    dayNames: ['日', '一', '二', '三', '四', '五', '六'], // Names of days
    // starting at
    // Sunday
    monthNames: ['一月', '二月', '三月', '四月', '五月', '六月',
			'七月', '八月', '九月', '十月', '十一月', '十二月'], // Names
    // of
    // months
    dateFormat: 'DMY/', // First three are day, month, year in the required
    // order, fourth is the separator, e.g. US would be
    // 'MDY/'
    yearRange: '-10:+10', // Range of years to display in drop-down, either
    // relative to current year (-nn:+nn) or absolute
    // (nnnn:nnnn)
    changeMonth: true, // True if month can be selected directly, false if only
    // prev/next
    changeYear: true, // True if year can be selected directly, false if only
    // prev/next
    ...
    autoPopUp: 'focus', // 'focus' for popup on focus, 'button' for trigger
    // button, or 'both' for either
    closeAtTop: true, // True to have the clear/close at the top, false to
    ...
    fieldSettings: null, // Function that takes an input field and returns a
    // set of custom settings for the calendar
    timeSeparators: [' ', ':'], // An array of time separators:
    //timeSeparators: [' ', ':', 'AM', 'PM'], // An array of time separators:
    // the first and second strings
    // are obligatory, while the
    // second and third strings
    // specify
    // the AM and PM strings if they are needed; the first parameter separates
    ...
    formatDate: function (day, month, year, hour, minute) {
        ...
        for (var i = 0; i < 3; i++) {
            dateString += this.dateFormat.charAt(3)
					+ (this.dateFormat.charAt(i) == 'D' ? (day < 10 ? '0' : '')
							+ day
							: (this.dateFormat.charAt(i) == 'M' ? (month < 10 ? '0'
									: '')
									+ month
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1
				&& this.timeSeparators[0] && hour != null && minute != null) {
            dateString += this.timeSeparators[0] + this.hourString(hour)
					+ this.timeSeparators[1]
					+ ((minute >= 10) ? minute : ('0' + minute));
            if (this.timeSeparators.length == 4 && hour < 12)
                dateString += this.timeSeparators[2];
            else if (this.timeSeparators.length == 4 && hour >= 12)
                dateString += this.timeSeparators[3];
        }
    ...
    parseDate: function (Text) {
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1) {
            ...
            var index = 0;
            while (index < currentTimeAndDate.length)
                if (currentTimeAndDate[index])
                    index++;
                else
                    currentTimeAndDate.splice(index, 1);
            if (currentTimeAndDate.length > 0)
                var currentDate = currentTimeAndDate[0].split(this.dateFormat
						.charAt(3));
            else
                var currentDate = [];
            if (currentTimeAndDate.length > 1)
                var currentTime = currentTimeAndDate[1]
						.split(this.timeSeparators[1]);
            else
                var currentTime = [];
        } else {
        ...
        if (currentDate.length == 3) {
            currentDay = parseInt(this.trimNumber(currentDate[this.dateFormat
					.indexOf('D')]));
            currentMonth = parseInt(this.trimNumber(currentDate[this.dateFormat
					.indexOf('M')])) - 1;
            currentYear = parseInt(this.trimNumber(currentDate[this.dateFormat
					.indexOf('Y')]));
        } else {
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1
            ...
            if (currentTime != null && currentTime.length == 2) {
                ...
                if (this.timeSeparators.length > 2) {
                    if (currentTime[1].indexOf(this.timeSeparators[2]) == currentTime[1].length
							- this.timeSeparators[2].length) {
                        if (currentHour == 12)
                            currentHour = 0;
                        currentMinute = parseInt(this.trimNumber(currentTime[1]
								.substring(0, currentTime[1].length
										- this.timeSeparators[2].length)));
                    } else {
                        if (currentHour < 12)
                            currentHour += 12;
    ...
    /*
    * Edited by Gregor Petrin to allow a custom input field as a parameter -
    * this can be used to display a date at start
    */
    selectDate: function (Input) {
        ...
        if (this.timeSeparators.length > 1)
            Input.val(this.formatDate(this.selectedDay, this.selectedMonth,
					this.selectedYear, this.selectedHour, this.selectedMinute));
        else
            Input.val(this.formatDate(this.selectedDay, this.selectedMonth,
					this.selectedYear));
    },
    ...
    showCalendar: function () {
        ...
        if (this.timeSeparators.length > 1) {
            ...
            if (this.timeSeparators.length == 4) {
                if (this.selectedHour == 12 || this.selectedHour == 0)
                    timeSelect += '<option value="12" selected="selected">12</option>';
                else
                    timeSelect += '<option value="12">12</option>';
                for (var a = 1; a < 12; a++) {
                    if (this.selectedHour == a
							|| (a == 12 && this.selectedHour == 0)
							|| (a + 12 == this.selectedHour))
                        timeSelect += '<option value="' + a
								+ '" selected="selected">' + a + '</option>';
                    else
                        timeSelect += '<option value="' + a + '">' + a
								+ '</option>';
                }
            } else {
                for (var a = 0; a < 24; a++) {
            ...
            if (this.timeSeparators.length == 4) {
                ...
                timeSelect += '<option value="AM"'
						+ ((this.selectedHour < 12) ? ' selected="selected"'
								: '') + '>' + this.timeSeparators[2]
						+ '</option>';
                timeSelect += '<option value="PM"'
						+ ((this.selectedHour >= 12) ? ' selected="selected"'
        ...
        var html = (this.closeAtTop ? '<div id="calendar_control">'
				...
				: '')
				+ '<div id="calendar_links"><a id="calendar_prev">'
				+ this.prevText
				+ '</a>'
				+ '<a id="calendar_current">'
				+ this.currentText
				+ '</a>'
				+ '<a id="calendar_next">'
				+ this.nextText + '</a></div>' + '<div id="calendar_header">';
        // alert(timeSelect);
        if (!this.changeMonth) {
        ...
        if (!this.changeYear) {
            ...
            year = (this.minDate ? Math.max(year, this.minDate.getFullYear())
					: year);
            endYear = (this.maxDate ? Math.min(endYear, this.maxDate
					.getFullYear()) : endYear);
        ...
        html += '</div>' + timeSelect + '<table id="calendar" cellpadding="0" cellspacing="0"><thead>' + '<tr class="calendar_titleRow">';
        for (var dow = 0; dow < 7; dow++) {
            html += '<td>' + (this.changeFirstDay ? '<a>' : '')
					+ this.dayNames[(dow + this.firstDay) % 7]
					+ (this.changeFirstDay ? '</a>' : '') + '</td>';
        }
        html += '</tr></thead><tbody>';
        var daysInMonth = this.getDaysInMonth(this.selectedYear,
				this.selectedMonth);
        this.selectedDay = Math.min(this.selectedDay, daysInMonth);
        var leadDays = (this.getFirstDayOfMonth(this.selectedYear,
				this.selectedMonth)
				- this.firstDay + 7) % 7;
        var currentDate = new Date(this.currentYear, this.currentMonth,
				this.currentDay);
        var selectedDate = new Date(this.selectedYear, this.selectedMonth,
				this.selectedDay);
        var printDate = new Date(this.selectedYear, this.selectedMonth,
				1 - leadDays);
        var numRows = Math.ceil((leadDays + daysInMonth) / 7); // calculate the
        // number of
        // rows to
        // generate
        var today = new Date();
        ...
        for (var row = 0; row < numRows; row++) { // create calendar rows
            ...
            for (var dow = 0; dow < 7; dow++) { // create calendar days
                var customSettings = (this.customDate ? this
						.customDate(printDate) : [true, '']);
                ...
                html += '<td class="calendar_daysCell'
						...
						(printDate.getTime() == currentDate.getTime() ? ' id="calendar_currentDay"'
								: // highlight current day
								(printDate.getTime() == today.getTime() ? ' id="calendar_today"'
										: ''))
						+ '>'
						+ // highlight today (if different)
						(otherMonth ? (this.showOtherMonths ? printDate
								.getDate() : '&nbsp;') : // display for other
                // months
		...
		(this.closeAtTop ? ''
				: '<div id="calendar_control"><a id="calendar_clear">'
						...
						'<a id="calendar_close">' + this.closeText
						+ '</a></div>');
        // add calendar to element to calendar Div
        //$('#calendar_div').empty().append(html).show(this.speed);  //ztf
        this.input[0].focus();
        this.setupActions();
    }, // end showCalendar
    ...
    showFor: function (target) {
        ...
        for (var i = 0; i < popUpCal.disabledInputs.length; i++) { // check not
            // disabled
        ...
        $.extend(popUpCal, (popUpCal.fieldSettings ? popUpCal
				.fieldSettings(input) : {}));
    ...
    doKeyDown: function (e) {
        ...
        } else if (e.keyCode == 36 && e.ctrlKey) { // display the calendar on
            // ctrl+home
    ...
    doKeyPress: function (e) {
        var chr = String.fromCharCode(e.charCode == undefined ? e.keyCode
				: e.charCode);
        return (chr < ' ' || chr == popUpCal.dateFormat.charAt(3) || (chr >= '0' && chr <= '9')); // only
        // allow
        // numbers
        // and
        // separator
    ...
    connectCalendar: function (target) {
        var $input = $(target);
        $input
				.after('<span class="calendar_append">' + this.appendText + '</span>');
        if (this.autoPopUp == 'focus' || this.autoPopUp == 'both') { // pop-up
            // calendar
            // when
            // in
            // the
        ...
        if (this.autoPopUp == 'button' || this.autoPopUp == 'both') { // pop-up
            // calendar
            // when
            // button
            // clicked
            $input
					...
					.after(
							this.buttonImageOnly ? '<img class="calendar_trigger" src="'
									+ this.buttonImage
									+ '" alt="'
									+ this.buttonText
									+ '" title="'
									+ this.buttonText + '"/>'
									: '<button class="calendar_trigger">' + (this.buttonImage != '' ? '<img src="'
											+ this.buttonImage
											+ '" alt="'
											+ this.buttonText
											+ '" title="'
            ...
            $((this.buttonImageOnly ? 'img' : 'button') + '.calendar_trigger',
					$input.parent('span')).click(this.showFor);
        }
    ...
    enableFor: function (inputs) {
        ...
        inputs.each(function () {
            ...
            $('../img.calendar_trigger', this).each(function () {
                $(this).css('opacity', '1.0');
            });
            var $this = this;
            popUpCal.disabledInputs = $.map(popUpCal.disabledInputs, function (
					value) {
    ...
    setupActions: function () {
        if (this.timeSeparators.length > 1) {
            $('#calendar_hour')
					.change(function () { // change hour
					    ...
					    popUpCal.adjustDate();
					    popUpCal.input.val(popUpCal.formatDate(
										popUpCal.selectedDay,
										popUpCal.selectedMonth,
										popUpCal.selectedYear,
										popUpCal.selectedHour,
										popUpCal.selectedMinute));
					}).click(this.selectMonthYear);
            $("#calendar_minute")
					.change(function () { // change minute
					    popUpCal.selecting = false;
        ...
        $('.calendar_titleRow a').click(function () { // change first day of
            // week
            for (var i = 0; i < 7; i++) {
    ...
    adjustDate: function (offset, period, dontShow) {
        if (this.timeSeparators.length > 1) {
            var date = new Date(this.selectedYear
					+ (period == 'Y' ? offset : 0), this.selectedMonth
					+ (period == 'M' ? offset : 0), this.selectedDay
					+ (period == 'D' ? offset : 0), this.selectedHour
					+ (period == 'H' ? offset : 0), this.selectedMinute
					+ (period == 'MIN' ? offset : 0));
        } else {
            var date = new Date(this.selectedYear
					+ (period == 'Y' ? offset : 0), this.selectedMonth
    ...
    setPos: function (targetObj, moveObj) {
        ...
        moveObj.css('position', 'absolute').css('left', coords[0] + 'px').css(
