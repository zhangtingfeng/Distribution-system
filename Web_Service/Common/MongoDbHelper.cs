    ...
    public class MongoDbHelper
    {
        ...
        public List<T> Find<T>(IMongoQuery query, string collectionName)
        {
            ...
            try
            {
                ...
                //if (null!=FieldsDocument)
                //{
                //    mongoCursor = mc.Find(query).SetFields(FieldsDocument);
                //}
                //else {
        ...
        public List<T> Find<T>(IMongoQuery query, int pageIndex, int pageSize, SortByDocument sortBy, string collectionName, FieldsDocument myFieldsDocument=null)
        {
            if (this._db == null)
            {
                return null;
            ...
            try
            {
                ...
                //按条件查询 排序 跳数 取数  
                if (myFieldsDocument != null)
                {
                    mongoCursor = mc.Find(query).SetSortOrder(sortBy).SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize).SetFields(myFieldsDocument);
                }
                else {
        ...
        /// <param name="sortBy">排序方式</param>  
        /// <returns>返回List列表</returns>  
        public List<T> Find<T>(IMongoQuery query, int pageIndex, int pageSize, SortByDocument sortBy,FieldsDocument myFieldsDocument = null)
        {
            string collectionName = typeof(T).Name;
            return this.Find<T>(query, pageIndex, pageSize, sortBy, collectionName, myFieldsDocument);
        }
        ...
        public long GetDataSize(string collectionName)
        {
            ...
            {
                MongoCollection<BsonDocument> mc = this._db.GetCollection<BsonDocument>(collectionName);
                return 5555555555;// mc.GetTotalStorageSize();
            }
            catch (Exception ex)
            {
                return 0;
